...
- [FASTQ解析ワークフロー](#fastq解析ワークフロー)
- [ancient / modern / auto の違い](#ancient--modern--auto-の違い)
- [BAM入力モード](#bam入力モード)
- [PCA/MDS](#pcamds)
  - [EIGENSOFTを使う通常経路](#eigensoftを使う通常経路)
  - [Python fallback](#python-fallback)
  - [PCA QCパラメータ](#pca-qcパラメータ)
- [出力](#出力)
- [チェックポイントと再開](#チェックポイントと再開)
- [サンプル並列実行](#サンプル並列実行)
- [実行オプション](#実行オプション)
- [進捗表示](#進捗表示)
- [ディレクトリ構成](#ディレクトリ構成)
...
- [トラブルシューティング](#トラブルシューティング)
  - [PCAでsample/siteが残らない](#pcaでsamplesiteが残らない)
  - [PCAで全sampleが100%欠損になる](#pcaで全sampleが100欠損になる)
  - [PCAのcallable sitesが0のまま進む](#pcaのcallable-sitesが0のまま進む)
  - [PCAが遅い・止まって見える](#pcaが遅い止まって見える)
...
### PCAで全sampleが100%欠損になる

ログに次のように出る場合は、単なる低depthではなく、PCA用matrixにalleleが1つも入っていません。

```text
No samples remain after PCA missingness filtering
sample missingness: min=100.00% / median=100.00% / max=100.00%
```

よくある原因は、空のPCA中間ファイルを再利用していることです。ログに `再開: 既存pseudo-haploid raw callsを使用します` や `再開: 既存pseudo-haploid matrixを使用します` が出ている場合は、PCA中間ファイルを削除してallele抽出から作り直してください。

```sh
rm data/results/ancient/cohort/pseudohaploid_raw_calls.tsv
rm data/results/ancient/cohort/pseudohaploid_matrix.tsv
rm data/results/ancient/cohort/pseudohaploid_matrix.filtered.tsv
```

すべてのPCA中間成果物を作り直したい場合は `--force` でも再実行できます。ただしsample側の `.done` も無視するため、必要な範囲だけ削除する方が短時間で済みます。

### PCAのcallable sitesが0のまま進む

最初のsampleが完了する前は、ダッシュボードの `callable 0 sites` は正常です。sample完了イベント後も `callable 0 sites` の場合は、PCA sitesとBAMのcontig名が一致していない可能性があります。

PCA sitesが `chr1`, `chr2` 形式なら、BAM headerも同じ形式である必要があります。確認例:

```sh
samtools idxstats data/results/ancient/PE-AncientHorses-01_S1/dedup/PE-AncientHorses-01_S1.dedup.sorted.bam | head
```

ここが `1`, `2` 形式なら、`chr` 付きのPCA sitesとは一致しません。参照ゲノム、BAM、PCA sitesのcontig名を同じ表記に揃えてください。

### PCAが遅い・止まって見える

PCA sitesが数百万から一千万以上ある場合、BAMからのpseudo-haploid allele抽出は時間がかかります。進捗表示の `サンプル` と `contig` が進んでいれば処理中です。ログファイルにはsample/contig単位の詳細も残ります。

速度確認だけを先にしたい場合は、小さめのPCA sitesファイルで試すと、環境やcontig名の問題を早く切り分けられます。

### PCAの結果は出たがsite数が少なすぎる

`data/results/<project>/cohort/pca_qc_summary.tsv` を確認してください。特に次の値を見ます。

- `input_sites`
- `kept_sites`
- `missingness_removed_sites`
- `monomorphic_removed_sites`
- `maf_removed_sites`
- `sex_chr_removed_sites`
- `ld_pruned_sites`

`missingness_removed_sites` が大きい場合は `--pca-max-site-missing` を緩めます。`monomorphic_removed_sites` が大きい場合は、対象cohort内で変異していないsiteが多い状態です。`sex_chr_removed_sites` が大きい場合は、`--pca-exclude-sex-chr` により性染色体siteが除外されています。
...
PCAだけを作り直したい場合は、sampleの `.done` はそのままにして、`data/results/<project>/cohort/` 配下の該当中間ファイルだけ削除する方が安全です。

### ログファイルが大きい

ログファイルは50MBごとに自動で切り分けられ、最大5世代まで保持されます。

```text
pipeline_ancient.log
pipeline_ancient.log.1
pipeline_ancient.log.2
```

さらに端末表示を減らしたい場合は `--no-progress` を指定してください。詳細ログは引き続き `data/logs/` に残ります。
